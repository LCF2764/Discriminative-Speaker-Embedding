...
Options:
  --mfcc-config <config-file>          # config passed to compute-raw-feats.
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_mfcc_${name}.JOB.log \
    ...
    compute-raw-feats $vtln_opts $write_utt2dur_opt --verbose=2 \
...
echo "$0: Succeeded creating raw waveform features for $name"
