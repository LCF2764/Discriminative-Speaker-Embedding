...
from model.resnet_1D import resnet_1D
...
class Trainer(object):
    ...
    def __init__(self, params, model_dir, single_cpu=False):
        ...
        if params.network_type == "tdnn":
            self.network = tdnn
        elif params.network_type == "resnet_1D":
            self.network = resnet_1D
        else:
    ...
    def build(self, mode, dim, loss_type=None, num_speakers=None, noupdate_var_list=None):
        ...
        if mode == "predict":
            ...
            with tf.name_scope("predict") as scope:
                ...
                feats, endpoints = self.entire_network(self.pred_features, self.params, is_training, reuse_variables)
                valid_loss, endpoints_loss = self.loss_network(feats, self.valid_labels, num_speakers, self.params, is_training, reuse_variables)
                if self.params.embedding_node == 'logits':
                    self.embeddings = endpoints_loss['logits']
                else:
    ...
    def predict(self, features):
        ...
        #valid_loss, endpoints_loss = self.loss_network(embeddings, self.valid_labels, num_speakers, self.params, is_training, reuse_variables)
        #embeddings = endpoints_loss['logits']
        if rank == 2:
    ...
    def get_finetune_model(self, excluded_list):
        ...
        ckpt = tf.train.get_checkpoint_state(self.model)
        ckpt_name = os.path.basename(ckpt.model_checkpoint_path)
        #sys.exit(ckpt.model_checkpoint_path)
