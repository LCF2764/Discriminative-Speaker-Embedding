...
def statistics_pooling(x, dim=-1, keepdim=False, unbiased=True, eps=1e-5):
    mean = x.mean(dim=dim, keepdim=True)
    var = torch.sum((x - mean)**2, dim=dim, keepdim=True)/x.size()[-1]
    std = torch.sqrt(var.clamp(min=eps))
    stats = torch.cat([mean.squeeze(dim=-1), std.squeeze(dim=-1)], dim=-1)
    if keepdim:
...
def high_order_statistics_pooling(x, dim=-1, keepdim=False, unbiased=True, eps=1e-2):
    ...
    var = torch.sum((x - mean)**2, dim=dim, keepdim=True)/x.size()[-1]
    std = torch.sqrt(var.clamp(min=eps))
    norm = (x - mean) / std.clamp(min=eps)
    skewness = norm.pow(3).mean(dim=dim)
    kurtosis = norm.pow(4).mean(dim=dim)
    stats = torch.cat([mean.squeeze(dim=dim), std.squeeze(dim=dim), skewness, kurtosis], dim=-1)
    if keepdim:
        stats = stats.unsqueeze(dim=dim)
    return stats
